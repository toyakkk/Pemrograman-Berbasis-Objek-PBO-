...
from services import IPaymentProcessor, ShoppingCart, DebitCardPayment
from models import Product

LOGGER = logging.getLogger('MAIN_APP')

class PosApp:
    """Kelas Orchestrator (Aplikasi Utama). Hanya mengkoordinasikan flow dan menerapkan DI."""
    def __init__(
            self,
    ...
    def print_receipt(self):
        ...
        for item in self.cart.get_items():
            LOGGER.info(f"{item.quantity}x {item.product.name} - Rp{item.subtotal:,.0f}")
        LOGGER.info(f"TOTAL: Rp{self.cart.total_price:,.0f}")
        LOGGER.info(f"PEMBAYARAN BERHASIL!")
        LOGGER.info("-----------------------\n")
